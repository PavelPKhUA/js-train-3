...
function stringToNumber(str) {
  let result = Number(str) // Використовуємо Number() для перетворення рядка в число
  return result // Повертаємо отримане число
}
...
function isAlmostSame(num1, num2) {
  return (
    num1 - num2 < Number.EPSILON &&
    num2 - num1 < Number.EPSILON
  )
...
function isUnsafeInteger(num) {
  return !isSafeInteger(num) // Використовуємо логічну оператор НЕ (!), щоб отримати протилежну відповідь від функції isSafeInteger()
}
...
function isAlmostZero(num) {
  return num < Number.MIN_VALUE && num > 0 // Перевіряємо, чи є число більше 0, але все ще менше за Number.MIN_VALUE
}
